...
class TestTaskViewSet:
    ...
    def test_create_task_integrity_error(self, mocker, user):
        mocker.patch(
            "apps.tasks.models.Task.objects.create",
            side_effect=IntegrityError("Integrity error"),
        )

        with pytest.raises(
            ValidationError,
            match="An error occurred while creating the task: Integrity error",
        ):
    ...
    def test_create_task_unexpected_error(self, mocker, user):
        ...
        with pytest.raises(
            ValidationError, match="An unexpected error occurred: Unexpected error"
        ):
            TaskService.create_task(
                owner=user,
                title="New Task",
    ...
    def test_update_task_does_not_exist(self, mocker):
        mocker.patch(
            "apps.tasks.models.Task.objects.get", side_effect=Task.DoesNotExist
        )
    ...
    def test_update_task_does_not_exist_to_user(self, api_client, user, category):
        task = baker.make(Task, owner=baker.make(User), category=category)
        api_client.force_authenticate(user=user)
        url = reverse("task-detail", args=[task.id])
        data = {
            "title": "Updated Task",
    ...
    def test_delete_task_user_forbidden(self, api_client, user):
        task = baker.make(Task, owner=baker.make(User))
        api_client.force_authenticate(user=user)
        url = reverse("task-detail", args=[task.id])
        response = api_client.delete(url)
        assert response.status_code == status.HTTP_404_NOT_FOUND
    ...
    def test_list_tasks(self, user, task):
        tasks = TaskService.list_tasks(owner=user)
